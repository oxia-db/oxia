...
func init() {
	Cmd.Flags().String("public", "", "Public address in host:port form")
	Cmd.Flags().String("internal", "", "Internal address in host:port form")
	Cmd.Flags().StringArray("label", nil, "Data server label in key=value form; repeatable")
...
func exec(cmd *cobra.Command, args []string) error {
	...
	createdDataServer, err := client.CreateDataServer(dataServer)
	if err != nil {
		return err
	}
	return dataserveroutput.WriteDataServer(cmd.OutOrStdout(), outputFormat, createdDataServer)
}
...
func parseLabels(values []string) (map[string]string, error) {
	...
	for _, value := range values {
		key, rawValue, found := strings.Cut(value, "=")
		if !found || key == "" {
