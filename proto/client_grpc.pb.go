...
const (
	...
	OxiaClient_Snapshot_FullMethodName            = "/io.streamnative.oxia.proto.OxiaClient/Snapshot"
)
...
func (c *oxiaClientClient) Snapshot(ctx context.Context, in *SnapshotRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[SnapshotResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &OxiaClient_ServiceDesc.Streams[7], OxiaClient_Snapshot_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type OxiaClient_SnapshotClient = grpc.ServerStreamingClient[SnapshotResponse]
...
func (UnimplementedOxiaClientServer) Snapshot(*SnapshotRequest, grpc.ServerStreamingServer[SnapshotResponse]) error {
...
func _OxiaClient_Snapshot_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(SnapshotRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(OxiaClientServer).Snapshot(m, &grpc.GenericServerStream[SnapshotRequest, SnapshotResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type OxiaClient_SnapshotServer = grpc.ServerStreamingServer[SnapshotResponse]
...
var OxiaClient_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "Snapshot",
			Handler:       _OxiaClient_Snapshot_Handler,
			ServerStreams: true,
		},
	},
