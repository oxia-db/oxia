...
func (s *shardController) waitForSplitComplete() {
	...
	for {
		...
		case <-s.ctx.Done():
			return
		case <-ticker.C:
			status := s.metadataProvider.LoadStatus()
...
func (s *shardController) onElectLeader(changeEnsembleAction *action.ChangeEnsembleAction) model.Server {
	...
	nsConfig, exist := s.metadataProvider.NamespaceConfig(s.namespace)
	if exist {
	...
	s.currentElection = NewShardElection(s.ctx, s.log, s.eventListener,
		s.metadataProvider, s.dataServerSupportedFeaturesSupplier, s.leaderSelector,
...
func (s *shardController) deleteShard() error {
	...
	s.metadataProvider.DeleteShardMetadata(s.namespace, s.shard)
...
func (s *shardController) SyncServerAddress() {
	...
	for _, candidate := range shardMeta.Ensemble {
		if newInfo, ok := s.metadataProvider.Node(candidate.GetIdentifier()); ok {
...
func (s *shardController) handlePeriodicTasks() {
	...
	s.metadataProvider.UpdateShardMetadata(s.namespace, s.shard, mutShardMeta)
