...
func NewSplitController(cfg SplitControllerConfig) *SplitController {
	...
	state := sc.statusResource.Get()
	ns, exists := state.Data.Namespaces[sc.namespace]
	if !exists {
...
func (sc *SplitController) abort() {
	...
	for _, childId := range []int64{sc.leftChildId, sc.rightChildId} {
		state := sc.statusResource.Get()
		if err := sc.statusResource.DeleteShard(sc.namespace, childId, state.Version); err != nil {
			sc.log.Warn("Failed to delete child shard during abort",
				slog.Int64("child-shard", childId),
				slog.Any("error", err),
			)
		}
	...
	if err := sc.updateParentMeta(func(meta *model.ShardMetadata) {
		...
		sc.log.Warn("Failed to clear parent split metadata during abort",
			slog.Any("error", err),
		)
	}
