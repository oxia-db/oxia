...
func (management *managementServer) PatchDataServer(_ context.Context, req *proto.PatchDataServerRequest) (*proto.PatchDataServerResponse, error) {
	...
	if req.DataServer.Identity.GetPublic() == "" && req.DataServer.Identity.GetInternal() == "" && req.DataServer.Metadata == nil {
		return nil, grpcstatus.Error(codes.InvalidArgument, "data server patch must specify at least one field to update")
	}
