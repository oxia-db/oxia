...
func (t testRaftClusterProvider) WaitToBecomeLeader(ctx context.Context) error {
	return t.leader.WaitToBecomeLeader(ctx)
}
...
func newTestRaftClusterProvider(t *testing.T) Provider {
	...
	for i := 0; i < 3; i++ {
		...
		go func() {
			p := trc.providers[idx]
			err := p.WaitToBecomeLeader(context.Background())
			assert.NoError(t, err)
