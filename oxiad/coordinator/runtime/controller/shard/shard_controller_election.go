...
// verifyChangeEnsemble verifies that the ensemble prepareChangeEnsemble moved
// the shard to cannot miss a committed entry. The data servers being removed
// are fenced but they are not candidates, so a committed entry that lives only
...
// ensemble, which is a candidate. That is RF/2+1 for an odd RF and RF/2 for an
// even one, where with RF=2 every committed entry is on both members and one
// fenced member is enough.
func (e *Election) verifyChangeEnsemble(candidatesStatus map[*proto.DataServerIdentity]*proto.EntryId) error {
	...
	e.logger.Warn(
		...
		slog.Any("unfenced-kept-members", unfenced),
	)
