...
func (mo *MetadataOptions) ApplyLegacyClusterConfigPath(configPath string) error {
	...
	case provider.NameConfigMap:
		if !strings.HasPrefix(configPath, legacyConfigMapClusterConfigPrefix) {
			return errors.New("deprecated cluster.configPath must use configmap:<namespace>/<name> when metadata provider is configmap")
		}
...
func (mpo ProviderOptions) IsSet() bool {
	return mpo.ProviderName != "" ||
		mpo.Kubernetes.Namespace != "" ||
		mpo.Kubernetes.StatusName != "" ||
		mpo.Kubernetes.ConfigName != "" ||
		mpo.File.Dir != "" ||
		mpo.File.StatusName != "" ||
		mpo.File.ConfigName != "" ||
		mpo.Raft.Address != "" ||
		mpo.Raft.DataDir != "" ||
		len(mpo.Raft.BootstrapNodes) > 0
}
...
type FileMetadata struct {
	...
	StatusName string `yaml:"statusName,omitempty" json:"statusName,omitempty" jsonschema:"description=File name for cluster status,example=cluster-status.json"`
	ConfigName string `yaml:"configName,omitempty" json:"configName,omitempty" jsonschema:"description=File name for cluster configuration,example=cluster.yaml"`
}
