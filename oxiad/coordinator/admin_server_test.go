...
func TestAdminServerGetDataServerByName(t *testing.T) {
	...
	require.NotNil(t, res.DataServer.DataServer)
	require.NotNil(t, res.DataServer.DataServer.Name)
	assert.Equal(t, serverName, *res.DataServer.DataServer.Name)
	assert.Equal(t, "public-2", res.DataServer.DataServer.PublicAddress)
	assert.Equal(t, "internal-2", res.DataServer.DataServer.InternalAddress)
...
func TestAdminServerGetDataServerByIdentifierFallback(t *testing.T) {
	...
	require.NotNil(t, res.DataServer.DataServer)
	require.NotNil(t, res.DataServer.DataServer.Name)
	assert.Equal(t, "internal-3", *res.DataServer.DataServer.Name)
	assert.Equal(t, "public-3", res.DataServer.DataServer.PublicAddress)
	assert.Equal(t, "internal-3", res.DataServer.DataServer.InternalAddress)
