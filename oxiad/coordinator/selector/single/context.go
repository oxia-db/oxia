...
func (so *Context) maybeGrouping() {
	so.candidateOnce.Do(func() {
		so.labelValueGroupedCandidates = util.GroupingCandidatesWithLabelValue(so.Candidates, so.CandidatesMetadata)
	})
	so.selectedOnce.Do(func() {
		so.labelGroupedSelectedLabelValues = util.GroupingValueWithLabel(so.selected, so.CandidatesMetadata)
	})
}
