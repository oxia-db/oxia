...
type ShardsDirector interface {
	...
	GetLeader(shardId int64) (lead.Controller, error)
	GetFollower(shardId int64) (follow.FollowerController, error)

	GetOrCreateLeader(namespace string, shardId int64) (lead.Controller, error)
	GetOrCreateFollower(namespace string, shardId int64, term int64) (follow.FollowerController, error)
...
func NewShardsDirector(config conf.Config, walFactory wal.Factory, kvFactory kv.Factory, provider rpc.ReplicationRpcProvider) ShardsDirector {
	sd := &shardsDirector{
		...
		kvFactory:              kvFactory,
		leaders:                make(map[int64]lead.Controller),
		followers:              make(map[int64]follow.FollowerController),
		...
		leadersCounter: metric.NewUpDownCounter("oxia_server_leaders_count",
			"The number of leader controllers in a node", "count", map[string]any{}),
		followersCounter: metric.NewUpDownCounter("oxia_server_followers_count",
			"The number of follower controllers in a node", "count", map[string]any{}),
	}

	return sd
}

func (s *shardsDirector) GetLeader(shardId int64) (lead.Controller, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *shardsDirector) GetOrCreateLeader(namespace string, shardId int64) (lead.Controller, error) {
	s.Lock()
	defer s.Unlock()
	...
	lc, err := lead.NewLeaderController(s.config, namespace, shardId, s.replicationRpcProvider, s.walFactory, s.kvFactory)
	if err != nil {
		return nil, err
	}
...
func (s *shardsDirector) GetOrCreateFollower(namespace string, shardId int64, term int64) (follow.FollowerController, error) {
	s.Lock()
	defer s.Unlock()
	...
	fc, err := follow.NewFollowerController(s.config, namespace, shardId, s.walFactory, s.kvFactory)
	if err != nil {
		return nil, err
	}
...
func (s *shardsDirector) DeleteShard(req *proto.DeleteShardRequest) (*proto.DeleteShardResponse, error) {
	...
	fc, err := follow.NewFollowerController(s.config, req.Namespace, req.Shard, s.walFactory, s.kvFactory)
	if err != nil {
		return nil, err
	}
