...
const (
	notificationsPrefix      = constant2.InternalKeyPrefix + "notifications"
...
func (n *Notifications) Modified(key string, versionId, modificationsCount int64) {
	if strings.HasPrefix(key, constant2.InternalKeyPrefix) {
		return
	}
...
func (n *Notifications) DeletedRange(keyStartInclusive, keyEndExclusive string) {
	if strings.HasPrefix(keyStartInclusive, constant2.InternalKeyPrefix) {
		return
	}
