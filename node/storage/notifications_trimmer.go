...
func newNotificationsTrimmer(ctx context.Context, namespace string, shardId int64, kv kv2.KV, notificationRetentionTime time.Duration, waitClose concurrent.WaitGroup, clock time2.Clock) *notificationsTrimmer {
...
func (t *notificationsTrimmer) readAt(offset int64) (time.Time, error) {
	_, res, closer, err := t.kv.Get(notificationKey(offset), kv2.ComparisonEqual)
	if err != nil {
