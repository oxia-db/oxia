...
func TestCoordinatorE2E(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		s, e := statusResource.Load()
		if e != nil {
			return false
		}
		shard := s.Namespaces[constant.DefaultNamespace].Shards[0]
...
func TestCoordinator_ShrinkCluster(t *testing.T) {
	...
	status, err := statusResource.Load()
	assert.NoError(t, err)
	leaderID := status.Namespaces["my-ns-1"].Shards[0].Leader.GetIdentifier()
