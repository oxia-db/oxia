...
func newServer(t *testing.T) (s *dataserver.Server, addr model.Server) {
	...
	var err error
	s, err = dataserver.New(t.Context(), commonoption.NewWatch(options))
...
func TestCoordinatorE2E(t *testing.T) {
	...
	coordinatorInstance, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(), func() (model.ClusterConfig, error) { return clusterConfig, nil }, nil, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_DeleteNamespace(t *testing.T) {
	...
	coordinatorInstance, err = coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(), func() (model.ClusterConfig, error) { return newClusterConfig, nil }, nil, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_DynamicallAddNamespace(t *testing.T) {
	...
	coordinatorInstance, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(), configProvider, configChangesCh, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_AddRemoveNodes(t *testing.T) {
	...
	configChangesCh := make(chan any)
	c, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(), configProvider, configChangesCh, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_RefreshServerInfo(t *testing.T) {
	...
	configChangesCh := make(chan any)
	c, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(), func() (model.ClusterConfig, error) {
		return clusterConfig, nil
	}, configChangesCh,
