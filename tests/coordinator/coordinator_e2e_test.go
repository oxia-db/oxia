...
func TestCoordinator_DeleteNamespace(t *testing.T) {
	...
	coordinatorInstance, err = coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(context.Background()), func() (model.ClusterConfig, error) { return newClusterConfig, nil }, nil, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_DynamicallAddNamespace(t *testing.T) {
	...
	coordinatorInstance, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(context.Background()), configProvider, configChangesCh, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_AddRemoveNodes(t *testing.T) {
	...
	configChangesCh := make(chan any)
	c, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(context.Background()), configProvider, configChangesCh, rpc2.NewRpcProvider(clientPool))
	assert.NoError(t, err)
...
func TestCoordinator_RefreshServerInfo(t *testing.T) {
	...
	configChangesCh := make(chan any)
	c, err := coordinator.NewCoordinator(metadataProvider, metadataProvider.RunElection(context.Background()), func() (model.ClusterConfig, error) {
		return clusterConfig, nil
	}, configChangesCh,
