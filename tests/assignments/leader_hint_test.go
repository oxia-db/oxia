...
func TestLeaderHintWithoutClient(t *testing.T) {
	...
	assert.Eventually(t, func() bool {
		shard := coordinatorInstance.StatusResource().Get().Status.Namespaces["default"].Shards[0]
		return shard.Status == model.ShardStatusSteadyState && shard.Leader != nil
	}, 20*time.Second, 100*time.Millisecond)

	target := sa1.Public
	status := coordinatorInstance.StatusResource().Get()
	shard := status.Status.Namespaces["default"].Shards[0]
...
func TestLeaderHintListWithoutClient(t *testing.T) {
	s1, sa1 := mock.NewServer(t, "s1")
	s2, sa2 := mock.NewServer(t, "s2")
	s3, sa3 := mock.NewServer(t, "s3")
	defer s1.Close()
	...
	// the shard assignments yet, so retry until the leader hint is present.
	assert.Eventually(t, func() bool {
		stream, err := clientRpc.List(t.Context(), &proto.ListRequest{Shard: &shardID})
		if err != nil {
			return false
		}
		_, err = stream.Recv()
...
func TestLeaderHintListWithClient(t *testing.T) {
	s1, sa1 := mock.NewServer(t, "s1")
	s2, sa2 := mock.NewServer(t, "s2")
	s3, sa3 := mock.NewServer(t, "s3")
	defer s1.Close()
	...
	keys, err := client.List(t.Context(), "/key1", "/key3")
	assert.NoError(t, err)
